...
- `server/` — ASP.NET Core 10 (net10.0) con **SignalR**. **Sin BBDD**: salas en memoria (`RoomManager`, `ConcurrentDictionary`); un `RoomSweeper` (BackgroundService) borra salas inactivas >5h.
  ...
  - `Hubs/GameHub.cs` — entrada de acciones; grupo SignalR = código de sala; grupo `public-watchers` = lista de salas públicas, incluyendo partidas en curso.
...
- `client/` — Angular 21 (standalone, signals, OnPush, control flow `@if/@for`) + TailwindCSS 4 (`@tailwindcss/postcss`, sin config file).
  ...
  - **Solo modo oscuro** (no hay theme service ni toggle). Los colores de la UI usan variables CSS mapeadas en `styles.css` vía `@theme`. La bandeja de la rueda (`.wheel-tray`) es fieltro oscuro fijo. Los dados propios se pueden ocultar manualmente y se muestran como interrogantes.
